...
from trust_bias.mLogging import logger
from trust_bias.preprocess import simulate_clicks
...
#         (self.doclist_ranges[:100])
  ...
  def load_batch_rel_clicks(self, qids):
    ...
    self.padded_labels = np.concatenate(padded_labels, axis=0)
    return np.concatenate(label_vector, axis=0), np.concatenate(click_rates, axis=0), np.concatenate(padding_mask, axis=0)
