...
from trust_bias import data_utils
from trust_bias.RelevanceProbs import RelNet
# from SIGIR20.mLogging import logger
from trust_bias.metrics import eval_output_unbiased, eval_output, eval_output_unbiased_denoised
...
flags.DEFINE_string(  'gold_inverse_propensities', "[1./((0.98-(i/100.))/(i+1.)-0.65/(i+1.)/(i+1.)) for i in range(20)]",
                      'ground truth inverse propensities.')
flags.DEFINE_string(  'gold_noises', "[0.65/(i+1.)/(i+1.) for i in range(10)]",
                      'ground truth noises.')
...
flags.DEFINE_string(  'learning_rate', '4e-3', 
                      'learning rate')
flags.DEFINE_string(  'learning_rate_decay_factor', '0.5**0.1', 
                      'learning rate decay factor. applied every on every batch.')
flags.DEFINE_string(  'min_learning_rate', '1e-5', 
                      'minimum learning rate after which no decaying is applied.')
flags.DEFINE_string(  'const_learning_rate_steps', '50', 
                      'number of steps before which learning rate is kept constant.')
...
flags.DEFINE_string(  'loss_fn', 'lambdaloss',
...
flags.DEFINE_string(  'click_policy_name', 'ideal_top10',
...
def create_model(data_set, checkpoint_path, forward_only, logger=None):
  model = RelNet(    checkpoint_path=checkpoint_path, 
                     ...
                     forward_only=forward_only, drop_out_probs=FLAGS.drop_out_probs,
                     learning_rate=eval(FLAGS.learning_rate), 
                     learning_rate_decay_factor=eval(FLAGS.learning_rate_decay_factor), 
                     ...
                     gold_inverse_propensities=FLAGS.gold_inverse_propensities, 
                     gold_noises=FLAGS.gold_noises, 
...
def train_model(  model_name, checkpoint_path, 
                  ...
                  min_learning_rate, learning_rate_decay_factor, const_learning_rate_steps, 
                  train_history_length=50, valid_history_length=1,
                  logger=None):
...
#   results = eval_output(y_true=y_true[:], y_pred=y_pred[:].numpy(), query_counts=np.diff(test_set.test_doclist_ranges()), report_dcg=report_dcg, k=10)
  results = eval_output(y_true=y_true[:], y_pred=y_pred[:], query_counts=np.diff(test_set.test_doclist_ranges()), report_dcg=report_dcg, k=10)
...
def train(click_data, logger):
  ...
  rel_model = train_model(  model_name='relevance', 
                            ...
                            min_learning_rate=eval(FLAGS.min_learning_rate), 
                            learning_rate_decay_factor=eval(FLAGS.learning_rate_decay_factor), 
                            const_learning_rate_steps=int(eval(FLAGS.const_learning_rate_steps)),
                            logger=logger) 
