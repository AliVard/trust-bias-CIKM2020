...
class DataFoldSplit(object):
  def __init__(self, datafold, name, doclist_ranges, feature_matrix, label_vector, data_raw_path):
  ...
  def random_subsample(self, subsample_size):
    ...
    return DataFoldSplit(None, self.name + str(qids), doclist_ranges, feature_matrix, label_vector, None)
...
class DataFold(object):
  ...
  def read_data(self):
    ...
    valid_raw_path = self.data_path + 'valid.txt'
...
#       print('reading from pickle')
    ...
    if not data_read:
      print('no pickle found, reading from raw files.')
      feature_map = {}
    ...
    self.data_fold_split_by_name = {'train': self.train, 'valid': self.valid, 'test': self.test}
