...
from trust_bias.preprocess import dataset
from trust_bias.preprocess import click_probs
...
if __name__ == '__main__':
  ...
  flags.DEFINE_string(  'click_probs_pickle_path', 'MSLR-WEB10K/Fold1/trust_1_top50.lgbm_20_3._.pkl',
...
class ClicksSplit:
  ...
  def _query_click_rates(self, query_index):
    ...
    return rates, self.ranks[s_i:e_i], shown_len
  ...
  def clean_and_sort(self):
    ...
    for qid in range(self.doclist_ranges.shape[0] - 1):
      rate, rank, shown_len = self._query_click_rates(qid)
      if shown_len > 0:
        cleaned_rate = np.zeros_like(rate)
        cleaned_rate[rank] = rate
        click_rates.append(cleaned_rate[:shown_len])
